...
def getFlatAtoms(ligand, flat_only = True, skip_hd = True):
    ...
    for l in atoms:
        ...
        elif (not atype in hydrogens) or (not skip_hd):
            data.append([atype, atomCoord(l), l])
    return data
...
def atomsToVector(at1, at2=None):
    at1 = atomCoord(at1)
    if at2: at2 = atomCoord(at2)
    return vector(at1, at2)

def vector(p1 , p2 = None): # TODO use Numpy?
    if not p2 == None:
        return array([p2[0]-p1[0],p2[1]-p1[1],p2[2]-p1[2]],'f')
    else:
        return array([p1[0], p1[1], p1[2] ], 'f' )
...
def vecAngle(v1, v2): # TODO remove and use Numpy
...
def validateEmail(string, localhost=False, exclude=''):
