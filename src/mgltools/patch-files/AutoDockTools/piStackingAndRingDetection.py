...
# atom geometry functions
#from AutoDockTools.HelperFunctionsN3P import atomCoord, avgCoord, getFlatAtoms, getAtype, pmvAtomStrip, dist, quickdist
# graph functions
#from AutoDockTools.HelperFunctionsN3P import getDistMatrix, makeGraph, intersect
# this will be likely changed to NumPy code
#from AutoDockTools.HelperFunctionsN3P import vector, norm, normalize, calcPlane, dot, vecAngle, vecSum
...
def getAromaticRes(receptor):
    ...
    for a in receptor:
        ...
        if a.startswith("HETATM"):
            ...
            if getAtype(a) in aromatic_res[rtype]:
                res_id = pmvAtomStrip(a).rsplit(":",1)[0] # A:GLY48:N -> A:GLY48
...
def findStacking(set1, set2 = None):
    ...
    MAXDIST_TSTACK = 5.0 # Angstroms
    MAXDIST_PSTACK = 4.0 # Angstroms
    ...
    MAXDIST = MAXDIST**2
    PTOL = 29.9 # degrees TODO test values on a wide set!
    TTOL = 14.9 # degrees TODO test values on a wide set!
            ...
            if not g1 == g2: # this is for intramolecular stackings (set1 = set2)
                ...
                if not [g1,g2] in p_accepted and not [g2,g1] in p_accepted:
                    if not [g1,g2] in t_accepted and not [g2,g1] in t_accepted:
                        d = quickdist(R1['centroid'], R2['centroid'])
                        if d <= MAXDIST_TSTACK:
                            #print "GOOD DIST: %2.3f" % sqrt(d),
                            pdist = abs(dot( R1['normal'][0], (R1['centroid']-R2['centroid']) ))
                            pangle = abs(vecAngle( R1['plane'], R2['plane']))
...
def findLigandRings(atoms):
    matrix = getDistMatrix(atoms)
    bdist = 2.0 # angstrom of bond dinstance
    graph = makeGraph(nodes_list = range(len(matrix)), distance_matrix = matrix, cutoff = bdist**2)
...
def searchPiGroupsLig(lig):
    ...
    atoms = getFlatAtoms(lig, flat_only = True)
...
if __name__ == "__main__":
    ...
    if not ligand_rings:
        ...
        for i in ligand_rings:
            res = ligand_rings[i]
            for n in res['normal']:
                v = vecSum( res['centroid'], n )
                out.write( makePdb(coord=v)+'\n' )
            out.write( makePdb(coord=res['centroid'])+'\n' )
        out.close()
